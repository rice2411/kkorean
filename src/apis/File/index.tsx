...
const FilesAPI = {
    ...
    deleteAssetByPublicId: async (
        publicId: string
    ): Promise<IAPI.ApiResponse<IAPI.BaseResponse> | unknown> => {
        try {
            const response = (await CloudinaryService.deleteAssetByPublicId(
                publicId
            )) as IAPI.ApiResponse<IAPI.BaseResponse>;
            return (
                response.data ||
                new Error(
                    response.error || "Unknown error occurred during deletion."
                )
            );
        } catch (error) {
    ...
    searchAssetsByPublicIdPrefix: async (
        ...
        try {
            const response =
                (await CloudinaryService.searchAssetsByPublicIdPrefix(
                    examId
