import { Box, Link, Paragraph } from "@/components/Atoms";
        ...
        >
            <Box className="text-sm font-normal text-gray-500 flex">
                ...
                <Paragraph className="font-semibold text-gray-900 mx-1">
                    {Math.min((page - 1) * DEFAULT_PAGE_SIZE + 1, length)} -{" "}
                    {Math.min(page * DEFAULT_PAGE_SIZE, length)}
                </Paragraph>
                of
            ...
            <Box className="inline-flex items-stretch -space-x-px cursor-pointer ">
                ...
                {Array(Math.ceil(length / DEFAULT_PAGE_SIZE))
                    .fill(null)
                    .map((_, index) => (
                        <Box
                            key={useId()}
