    ...
    >
      <Box className="text-sm font-normal text-gray-500 flex">
        ...
        <Paragraph className="font-semibold text-gray-900 mx-1">
          ...
          - {Math.min(page * PAGINATION_CONSTANTS.DEFAULT_PAGE_SIZE, length)}
        </Paragraph>
        of
      ...
      <Box className="inline-flex items-stretch -space-x-px cursor-pointer ">
        ...
        >
          ...
          <Svg
            ...
            fill="currentColor"
            viewBox="0 0 20 20"
            src={FileHelpers.getLocalFile("pagination-left-arrow", "path")}
          />
        </Box>
        {Array(Math.ceil(length / PAGINATION_CONSTANTS.DEFAULT_PAGE_SIZE))
          .fill(null)
          .map((_, index) => (
            <Box
              key={index}
              className={page === index + 1 ? activeClass : inactiveClass}
              onClick={() => {
                setPage(index + 1);
              }}
            >
        ...
        >
          ...
          <Svg
            ...
            fill="currentColor"
            viewBox="0 0 20 20"
            src={FileHelpers.getLocalFile("pagination-right-arrow", "path")}
          />
        </Box>
      </Box>
    </Box>
