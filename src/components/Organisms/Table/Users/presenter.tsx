import { AvatarName, Box, Input, Paragraph, Svg } from "@/components/Atoms";
      ...
      <Box className="bg-white relative overflow-hidden mt-4">
        <Box className="flex flex-col md:flex-row items-stretch md:items-center md:space-x-3 space-y-3 md:space-y-0 justify-between mx-4 py-4 border-t">
          ...
          <Box className="w-full md:w-auto flex flex-col md:flex-row space-y-2 md:space-y-0 items-stretch md:items-center justify-end md:space-x-3 flex-shrink-0">
            <Box
              onClick={() => {
                handleOpenModalUser(MODAL_CONSTANTS.EModalType.CREATE);
              }}
            ...
            {users && users.length > 0 && (
              <tbody>
                {users
                  ...
                  .filter(
                    (user) =>
                      user.fullName.toLowerCase().includes(searchContent) ||
                  ...
                  .map((user, index) => (
                    <tr
                      ...
                      onClick={(e: React.MouseEvent) => {
                        ...
                        if (target.id.includes("action")) return;
                        handleOpenModalUser(
                    ...
                    >
                      ...
                      >
                        <AvatarName size={10} name={user.fullName} />
                        <div className="ps-3">
                        ...
                        <div className="flex items-center">
                          <div
                            className={`h-2.5 w-2.5 rounded-full me-2 ${
                              user.isDisabled ? "bg-red-500" : "bg-green-500"
                            }`}
                          />
                        ...
                        {user.role === CONFIG_CONSTANTS.EUserRole.ADMIN ? (
                          ...
                          <>
                            <Box
                              id="action-btn-1"
                              onClick={() =>
                                handleConfirmUpdateAccountStatus(user)
                              }
                              className={`${
                                user.isDisabled
                            ...
                            <Box
                              id="action-btn-2"
                              onClick={() => handleImportantConfirm(user)}
                            ...
                            <Box
                              id="action-btn-3"
                              onClick={() => handleConfirmResetPassword(user)}
        ...
        <Pagination length={users.length} page={page} setPage={setPage} />
      </Box>
