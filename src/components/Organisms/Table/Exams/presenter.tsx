                  ...
                  .map((exam, index) => (
                    <tr key={exam.id} className="border-b  hover:bg-gray-100 ">
                      ...
                      <td className="px-4 py-3 font-medium text-gray-900 whitespace-nowrap ">
                        <Paragraph
                          className={`${
                            exam.plan === EXAM_CONSTANTS.EXAM_PLAN.FREE
                              ? " bg-green-100 text-green-800"
                              : "bg-primary-100 text-primary-800"
                          } text-xs font-medium px-2 py-0.5 rounded w-max`}
                        >
                          {ExamUtils.getExamPlan(exam.plan || 0)}
                        </Paragraph>
                      </td>
                      <td className="px-4 py-3">
                        <Paragraph
                      ...
                      <td className="px-4 py-3 font-medium text-gray-900 whitespace-nowrap ">
                        <Box className="flex items-center">
                          <Box
                            className={`${
                              exam.level === EXAM_CONSTANTS.EXAM_LEVEL.EASY
                                ? "bg-green-500"
                                : exam.level ===
                                  EXAM_CONSTANTS.EXAM_LEVEL.MEDIUM
                                ? "bg-yellow-500"
                                : "bg-red-500"
                            } h-4 w-4 rounded-full inline-block mr-2 `}
                          />
                          {ExamUtils.getExamLevel(exam.level || 0)}
                        </Box>
                      </td>
                      <td className="px-4 py-3 font-medium text-gray-900 whitespace-nowrap ">
                        <Box className="flex items-center space-x-4">
                          <Box
                            onClick={() => {
                              handleOpenWindowExplorer(
                                exam,
                                FILE_CONSTANTS.UPLOAD_TYPE.IMAGE
                              );
                            }}
                          ...
                          {exam.type === EXAM_CONSTANTS.EXAM_TYPE.LISTENING && (
                            <Box
                              onClick={() => {
                                handleOpenWindowExplorer(
                                  exam,
                                  FILE_CONSTANTS.UPLOAD_TYPE.AUDIO
                                );
                              }}
