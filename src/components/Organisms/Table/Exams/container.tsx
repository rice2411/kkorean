...
const TableExamsContainer= () => {
  ...
  const onUploadImages = async (event: React.ChangeEvent<HTMLInputElement>) => {
    try {
      ...
      const invalidFiles = selectedFiles.filter((file: File) => {
        const fileExtension = file.name.split(".").pop()?.toLowerCase() || "";
        return !FILE_CONSTANTS.EAllowedExtensions.IMAGE.includes(fileExtension);
      });
  ...
  const onUploadExcel = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (e: ProgressEvent<FileReader>) => {
      if (e.target && e.target.result) {
        ...
        const secondSheetName: any = XLSX.utils.sheet_to_json(
        ...
        payload.description = secondSheetName[0];
    ...
    if (uploadRef.current) {
      switch (type) {
        case FILE_CONSTANTS.EUploadType.IMAGE:
          ...
          uploadRef.current.accept = FILE_CONSTANTS.EAllowedExtensions.IMAGE;
          break;
        case FILE_CONSTANTS.EUploadType.AUDIO:
          uploadRef.current.accept = FILE_CONSTANTS.EAllowedExtensions.AUDIO;
          break;
        case FILE_CONSTANTS.EUploadType.EXCEL:
          uploadRef.current.accept = FILE_CONSTANTS.EAllowedExtensions.EXCEL;
          break;
      }
      setExam(exam);
