...
const ExamProgress = () => {
  ...
  const countExam = (type: EExamType) => {
    return (
      exams.filter(
        (exam) =>
  ...
  return (
    <Box className="grid grid-cols-1 gap-4 md:grid-cols-3 md:gap-20 border border-stone-300 rounded-md p-4 mt-8">
      <Box>
        ...
        />
        {countExam(EExamType.LISTENING)}
        <Paragraph className="inline text-xs text-gray-500">
          /{exams.filter((exam) => exam.type === EExamType.LISTENING).length} đã
          hoàn thành
        </Paragraph>
      </Box>
      <Box>
        ...
        />
        {countExam(EExamType.READING)}
        <Paragraph className="inline text-xs text-gray-500">
          / {exams.filter((exam) => exam.type === EExamType.READING).length} đã
          hoàn thành
        </Paragraph>
      </Box>
      <Box>
